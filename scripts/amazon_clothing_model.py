...
def main():
    # loading sampled amazon clothing dataset
    dataframe_arrow = load_from_disk("datasets/amazon_clothing_reviews_10k")
    dataframe = dataframe_arrow.to_pandas()
    dataframe.to_csv("datasets/csvs/amazon_clothing.csv", index=False)
    ...
    model = Pipeline(steps=[
        ...
        ("classifier", LogisticRegression(
            class_weight="balanced", # for class inbalance
            max_iter=2000, # higher iterations for convergence
            random_state=33 # random state, 33 is not technically important, just my favourite numbers
        ))
    ])
    ...
    coef_df.to_csv("coefs/amazon_clothing_coefs.csv", index=False)
